...
namespace RoamingApp.Tests
{
    public abstract class RoamingAppTestBase : AbpIntegratedTestBase<RoamingAppTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected RoamingAppTestBase()
        {
        ...
        protected void UsingDbContext(Action<RoamingAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<RoamingAppDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<RoamingAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<RoamingAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<RoamingAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<RoamingAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<RoamingAppDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<RoamingAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<RoamingAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<RoamingAppDbContext, Task<T>> func)
        {
            T result;
